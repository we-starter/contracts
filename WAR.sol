...
contract WAR is MappableToken {
	function __WAR_init(address creator_, address factory_, address mine_, address mine2_, address eco_, address team_, address seed_, address private_, address public_, address lbp_, address liqudity_) external initializer {
        __Context_init_unchained();
		__ERC20_init_unchained("WeStarter", "WAR");
		__MappableToken_init_unchained(factory_, creator_);
		__WAR_init_unchained(mine_, mine2_, eco_, team_, seed_, private_, public_, lbp_, liqudity_);
	}
	
	function __WAR_init_unchained(address mine_, address mine2_, address eco_, address team_, address seed_, address private_, address public_, address lbp_, address liqudity_) public initializer {
		_mint(mine_,            30_000_000 * 10 ** uint256(decimals()));
		_mint(mine2_,           35_000_000 * 10 ** uint256(decimals()));
		...
		_mint(private_,          5_000_000 * 10 ** uint256(decimals()));
		_mint(public_,           1_000_000 * 10 ** uint256(decimals()));
		_mint(lbp_,              1_000_000 * 10 ** uint256(decimals()));
		_mint(liqudity_,         3_000_000 * 10 ** uint256(decimals()));
	}
}



contract WARLOCK is ERC20UpgradeSafe, Configurable {
    using SafeERC20 for IERC20;
	...
	function __WARLOCK_init(address governor_) public initializer {
		__Governable_init_unchained(governor_);
        __Context_init_unchained();
		__ERC20_init_unchained("WAR LOCKing", "WARLOCK");
		_setupDecimals(18);
	}
